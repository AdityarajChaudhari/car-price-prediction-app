...
print("So we have trained four different models lets compare them!!")

models = {
    'Algorithm' : ['GradientBoostingRegressor','DecisionTreeRegressor','RandomForestRegressor','BaggingRegressor'],
    'Train_Acc' : [round(gbr_best.score(x_train,y_train),4),round(randomcv_dt_best.score(x_train,y_train),4),round(randomcv_rfr_best.score(x_train,y_train),4),round(randomcv_bgr_best.score(x_train,y_train),4)],
    'Test_Acc'  : [round(gbr_best.score(x_test,y_test),4),round(randomcv_dt_best.score(x_test,y_test),4),round(randomcv_rfr_best.score(x_test,y_test),4),round(randomcv_bgr_best.score(x_test,y_test),4)]
}
...
pickle.dump(gbr_best,open('model.pkl','wb'))
